...
const updateFormData = async (req, res) => {
  ...
  try {
    ...
    query += Object.keys(formData)
      .filter(key => existingColumns.includes(key.replace(/[^a-zA-Z0-9_ ()]/g, '')))
...
const updateAutofillData = async (req, res) => {
  const { formName, uniqueId, updatedData } = req.body;
  try {
    const pool = await connectToDatabase(db3);
    const request = pool.request();
    ...
    // Construct the update query
    let query = `UPDATE [${formName.replace(/'/g, "''")}] SET `;
    query += Object.keys(updatedData).map((key) => `[${key}] = @${key}`).join(', ');
    query += ` WHERE UniqueId = @uniqueId`;

    // Add the uniqueId parameter
    request.input('uniqueId', sql.Int, uniqueId);

    // Execute the query
    await request.query(query);

    res.status(200).send('Autofill data updated successfully');
  } catch (error) {
    console.error('Error updating autofill data:', error);
    res.status(500).send('Error updating autofill data');
  }
};

const updateFetchedData = async (req, res) => {
  const { formName, uniqueId, updatedData } = req.body;
  try {
    const pool = await connectToDatabase(db3);
    const request = pool.request();
    ...
    res.status(200).send('Fetched data updated successfully');
  } catch (error) {
    console.error('Error updating fetched data:', error);
    res.status(500).send('Error updating fetched data');
  }
};
