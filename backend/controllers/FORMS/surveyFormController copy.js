...
const getSurveyFormFirstQuestion = async (req, res) => {
  const { formTypeId, surveyFormName } = req.query;
  try {
    const pool = await connectToDatabase(db3);
        const result = await pool.request()
      .input('formTypeId', sql.Int, formTypeId)
      .input('surveyFormName', sql.NVarChar, surveyFormName)
      .query(`
        SELECT TOP 1 * FROM SurveyFormSpecifications
        WHERE FormTypeId = @formTypeId AND SurveyFormName = @surveyFormName
    ...
    console.error(err);
    res.status(500).send('Error fetching survey form first question');
  }
};
