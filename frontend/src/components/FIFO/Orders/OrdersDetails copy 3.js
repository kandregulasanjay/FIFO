...
const OrderDetails = () => {
    ...
    const [, setPendingQty] = useState(0);
    const [, setShowPendingPopup] = useState(false);
    ...
    useEffect(() => {
        const fetchDetails = async () => {
            try {
                ...
                const totalAllocated = data.reduce((sum, item) => sum + item.allocated_quantity, 0);
                const requiredQty = data[0].pickslip_qty;
                if (totalAllocated < requiredQty) {
                    setPendingQty(requiredQty - totalAllocated);
    ...
    const handleConfirmSubmit = async () => {
        ...
        try {
            ...
            if (response.success) {
                ...
                setOrderDetails([]);
                setOrderCompleted(true); // Mark as completed
                if (response.pendingQty > 0) {
                    setPendingQty(response.pendingQty);
                    setShowPendingPopup(true);
                }
                // Download PDF automatically after success
                try {
                    const data = await getPickslipPDF(pickslipNumber);
                    if (data.error) throw new Error(data.error);
                    if (!Array.isArray(data.items)) throw new Error("Invalid data format: items is not an array.");
                    generatePickslipPDF(
                        pickslipNumber,
                        data.items,
                        data.customer_name,
            ...
            submittingRef.current = false; // Allow future submissions if needed
        }
    };
