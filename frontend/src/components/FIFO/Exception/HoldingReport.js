...
import { fetchHoldingReportData } from '../../../api/api';
...
const ReceiptExceptionPage = () => {
  ...
  const fetchData = async (invoiceNum = '') => {
    setLoading(true);
    try {
      const response = await fetchHoldingReportData(invoiceNum);
      setData(response);
  ...
  return (
    <div className="p-4">
      <div className="flex flex-col sm:flex-row justify-between items-center mb-4 gap-2">
        <h2 className="text-xl font-semibold">Holding Report</h2>
        <form onSubmit={handleSearch} className="flex items-center gap-2">
          ...
          <button
            type="button"
            onClick={() => { setInvoiceNumber(''); fetchData(); }}
