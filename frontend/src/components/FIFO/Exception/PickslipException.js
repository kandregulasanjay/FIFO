...
import { fetchPickslipExceptionData } from '../../../api/api';
import { ToastContainer, toast } from 'react-toastify';
import 'react-toastify/dist/ReactToastify.css';
import CircularProgress from '@mui/material/CircularProgress';
import Box from '@mui/material/Box';

const PickslipExceptionPage = () => {
  const [data, setData] = useState([]);
  const [columns, setColumns] = useState([]);
  const [loading, setLoading] = useState(false);
  const [pickslipNumber, setPickslipNumber] = useState('');
  const [currentPage, setCurrentPage] = useState(1);
  const rowsPerPage = 10;

  // Fetch data (optionally filtered by pickslip_number)
  const fetchData = async (pickslipNum = '') => {
    setLoading(true);
    try {
      const response = await fetchPickslipExceptionData(pickslipNum);
      setData(response);
  ...
  // Handle pickslip_number search
  const handleSearch = (e) => {
    e.preventDefault();
    fetchData(pickslipNumber);
    setCurrentPage(1);
  };
  ...
  return (
    <div className="p-4">
      <div className="flex flex-col sm:flex-row justify-between items-center mb-4 gap-2">
        <h2 className="text-xl font-semibold">Pickslip Report</h2>
        <form onSubmit={handleSearch} className="flex items-center gap-2">
          <input
            type="text"
            placeholder="Search by Pickslip Number"
            value={pickslipNumber}
            onChange={e => setPickslipNumber(e.target.value)}
          ...
          <button
            type="button"
            onClick={() => { setPickslipNumber(''); fetchData(); }}
...
export default PickslipExceptionPage;