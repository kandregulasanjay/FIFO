...
export const generateHoldingPDF = (pickslipNumber, orderDetails, customerName, issuedAt, type, invoiceNumber) => {
    const doc = new jsPDF();
    let yPos = 18;
    ...
    const addHeader = () => {
        ...
        doc.text(`Printed Date: ${new Date().toLocaleDateString()}`, 15, yPos);
        doc.text(`Issued Date: ${formatDate(issuedAt)}`, 140, yPos);
        yPos += 8;
    ...
    if (tableData.length === 0) {
        ...
        doc.autoTable({
            startY: yPos + 5,
            head: [[ 'Invoice Number', 'Make', 'Item Code', 'Bin Location', 'Ordered Qty', 'Issued Qty', 'Remaining Qty', 'Issued Date']],
            body: tableData, 
            theme: 'grid',
